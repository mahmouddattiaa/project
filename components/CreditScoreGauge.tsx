...
export function CreditScoreGauge({ score, size = 200 }: CreditScoreGaugeProps) {
  ...
  const scoreAngle = 180 + ((score - 300) / 550) * 180;
  const scoreRadians = scoreAngle * (Math.PI / 180);
    ...
    <View style={styles.container}>
      <Svg width={size} height={size / 2 + 40} style={styles.svg}>
        ...
        {ranges.map((range, index) => {
          const startAngle = 180 + ((range.min - 300) / 550) * 180;
          const endAngle = 180 + ((range.max - 300) / 550) * 180;
